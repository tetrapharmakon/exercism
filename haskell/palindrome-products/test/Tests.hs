  ...
  where
    ...
    test (desc, minFactor, maxFactor, Nothing) =
      describe desc $ do
        describe "smallestPalindrome" $
          it "result" $
          smallestPalindrome minFactor maxFactor `shouldBe` Nothing
    ...
    cases =
      [ ( "palindromes from single digit factors"
        , 1
        , 9
        , Just (1, [(1, 1)], 9, [(1, 9), (3, 3)]))
      , ( "palindromes from double digit factors"
        , 10
        , 99
        , Just (121, [(11, 11)], 9009, [(91, 99)]))
      , ( "palindromes from triple digit factors"
        , 100
        , 999
        , Just (10201, [(101, 101)], 906609, [(913, 993)]))
      , ( "palindromes from four digit factors"
        , 1000
        , 9999
        , Just (1002001, [(1001, 1001)], 99000099, [(9901, 9999)]))

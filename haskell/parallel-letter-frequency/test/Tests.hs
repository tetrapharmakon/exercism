...
specs = do
  ...
  describe "frequency" $ do
    it "no texts mean no letters" $ frequency 1 [] `shouldBe` empty
    it "one letter" $ frequency 1 ["a"] `shouldBe` singleton 'a' 1
    it "case insensitivity" $ frequency 1 ["aA"] `shouldBe` singleton 'a' 2
    ...
    it "many times the same text gives a predictable result" $
      frequency 1 (replicate 1000 "abc") `shouldBe`
